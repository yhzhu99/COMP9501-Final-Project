...
4.  **Consider Available Tools:** Review the MCP tools that the execution agent has access to (shown below). While you cannot directly use these tools, you should create plans that instruct the execution agent to use them through Python code when appropriate. Tools can be directly called in the form of functions in the Python environment without the need for additional imports.
...
6.  **Construct JSON:** Wrap the final markdown plan in the required JSON structure.
